...
final class L10n: ObservableObject {
    ...
    private static let ru: [String: String] = [
        ...
        "status.test.disclaimer": "Кнопки ниже только посылают команду через approval-pipeline, чтобы проверить нотификацию и окно подтверждения. Сами команды НЕ выполняются.",
        ...
        "settings.direct_toggle": "Сразу показывать окно подтверждения",
        "settings.direct_description": "Без системного уведомления — окно с командой выскочит сразу при поступлении запроса. Удобно если ты постоянно за компьютером и не хочешь лишних кликов.",
    ...
    private static let en: [String: String] = [
        ...
        "status.test.disclaimer": "These buttons only push the command through the approval pipeline to verify the notification + confirmation window. The commands themselves are NOT executed.",
        ...
        "settings.direct_toggle": "Show confirmation window immediately",
        "settings.direct_description": "Skip the system notification — the request window pops up directly. Useful when you're always at the computer and don't want to click through banners.",
